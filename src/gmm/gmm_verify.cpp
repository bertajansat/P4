...
float verify(const GMM &gmm_candidate, const fmatrix &dat)
{
	...
	return score;
	/// \ DONE Implementada la verificación de la score basada en el gmm del candidato
}
...
float verify(const GMM &gmm_candidate, const GMM & gmm_world, const fmatrix &dat, float &lprobcand, float &lprobbackground)
{
	...
	return score;
	/// \DONE Implementada la verificación tomando como referencia el model del mundo
}
